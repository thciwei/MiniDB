...
import com.thciwei.minidb.backend.im.BPlusTree;
import com.thciwei.minidb.backend.parser.statement.SingleExpression;
import com.thciwei.minidb.backend.tm.TransactionManagerImpl;
import com.thciwei.minidb.backend.utils.Panic;
import com.thciwei.minidb.backend.utils.ParseStringRes;
...
public class Field {
    ...
    private Field parseSelf(byte[] raw) {
        ...
        Parser.parseLong(Arrays.copyOfRange(raw, position, position + 8));
        if (index != 0) {
            try {
    ...
    private void persistSelf(long xid) throws Exception {
        ...
        this.uid = ((TableManagerImpl) tb.tbm).dm.insert(xid, Bytes.concat(nameRaw, typeRaw, indexRaw));
    }
